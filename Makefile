...
jshell:
	jshell --enable-preview \
		--start PRINTING \
		--start JAVASE \
		--class-path `mvn dependency:build-classpath | grep -A1 'Dependencies classpath' | tail -1`
...
module-jshell: module-delombok
	jshell --enable-preview \
		--start PRINTING \
		--start JAVASE \
		--class-path `mvn dependency:build-classpath | grep -m2 -A1 'Dependencies classpath' | head -n2 | tail -1`:$(A)/target/generated-sources/delombok
