...
public class WithoutAbstractionsTest extends AbstractHibernateTest {
  ...
  void shouldCorrectSaveAndLoadUserWithExpectedQueriesCount(boolean userDetachedBeforeGet) {
    try (Session session = sessionFactory.openSession()) {
      ...
      // Реальная вставка произошла в момент коммита транзакции
  ...
  @DisplayName(" показывать что с загруженным с помощью load не существующим объектом можно номально работать после того, как он был добавлен в БД")
  @Test
  ...
  void shouldUpdatePersistentEntityInDBWhenChangedFieldsInTransaction() {
    ...
    // Проверка, что имя загруженного пользвателя соответсвует тому, что дали после сохранения
